...
import type { ActiveTab } from '@/components/page/home-client-page';
...
const DISCORD_INVITE_LINK = "https://discord.gg/YP9UraDH4k";
...
export default function Header({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const fetchedNotifications = snapshot.docs.map(doc => {
          ...
          if (!data.title || !data.message || !data.type || !data.createdAt) return null;
  ...
  }, [isLoggedIn, currentUser?.email, toast]);
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    if (notification.lessonId) {
        setActiveTab('aulas');
        router.push(`/?lessonId=${notification.lessonId}#main-content`);
    }
  };
          ...
          <>
             <DropdownMenu>
               <DropdownMenuTrigger asChild>
                 <Button
                   variant="ghost"
                   className="relative text-muted-foreground hover:text-primary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 rounded-full flex items-center gap-2 px-2 md:px-3 py-1 h-8 md:h-9 group"
                 >
                   <Avatar className="h-6 w-6 md:h-7 md:w-7 border">
                       <AvatarImage src={userAvatarUrl || undefined} alt={userName || 'User Avatar'} />
                       <AvatarFallback>{userName ? userName.substring(0, 2).toUpperCase() : <User className="h-3 w-3 md:h-4 md:w-4" />}</AvatarFallback>
                   </Avatar>
                    <div className="hidden sm:flex items-center gap-1.5">
                       <span className="text-xs font-semibold text-foreground truncate leading-tight">{userName || 'Usuário'}</span>
                       {isAdmin ? (
                         <Badge variant="default" className="bg-primary text-primary-foreground px-1 py-0 text-[9px] font-bold flex items-center w-fit"><Crown className="h-2 w-2 mr-0.5" /> ADMIN</Badge>
                       ) : (
                         <Badge variant="secondary" className="text-[9px] px-1 py-0 w-fit leading-tight">{displayRank}</Badge>
                       )}
                    </div>
                    <Settings className="h-3 w-3 md:h-4 md:w-4 text-muted-foreground group-hover:text-primary ml-1 flex-shrink-0" />
                 </Button>
               </DropdownMenuTrigger>
               <DropdownMenuContent align="end" className="w-56 bg-card border-border shadow-lg">
                 <DropdownMenuLabel className="px-2 py-2">
                   <div className="flex items-center gap-2">
                     <Avatar className="h-8 w-8 border">
                       <AvatarImage src={userAvatarUrl || undefined} alt={userName || 'User Avatar'} />
                       <AvatarFallback>{userName ? userName.substring(0, 2).toUpperCase() : <User className="h-4 w-4" />}</AvatarFallback>
                     </Avatar>
                     <div className="flex flex-col">
                       <span className="text-sm font-semibold text-foreground truncate">{userName || 'Usuário'}</span>
                       {isAdmin ? (
                         <Badge variant="default" className="bg-primary text-primary-foreground px-1.5 py-0 text-[10px] font-bold mt-0.5 flex items-center w-fit"><Crown className="h-2.5 w-2.5 mr-1" /> ADMIN</Badge>
                       ) : (
                 ...
                 <DropdownMenuSeparator />
                 <DropdownMenuItem asChild><Link href="/profile" className="flex items-center cursor-pointer text-sm"><UserCircle className="mr-2 h-4 w-4" /><span>Ver Perfil</span></Link></DropdownMenuItem>
                 <DropdownMenuItem asChild><Link href="/premium" className="flex items-center cursor-pointer text-sm"><Star className="mr-2 h-4 w-4" /><span>Planos Premium</span></Link></DropdownMenuItem>
                  <DropdownMenuItem asChild><Link href={DISCORD_INVITE_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center cursor-pointer text-sm"><LifeBuoy className="mr-2 h-4 w-4" /><span>Suporte</span></Link></DropdownMenuItem>
                 ...
                 <DropdownMenuSeparator />
                 <DropdownMenuItem onClick={onSignOut} className="text-destructive focus:text-destructive focus:bg-destructive/10 cursor-pointer flex items-center text-sm" disabled={isSigningOut}>
                   {isSigningOut ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /><span>Saindo...</span></>) : (<><LogOut className="mr-2 h-4 w-4" /><span>Sair</span></>)}
                 </DropdownMenuItem>
               </DropdownMenuContent>
             </DropdownMenu>
             ...
             <Popover open={isPopoverOpen} onOpenChange={handlePopoverOpenChange}>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="relative text-muted-foreground hover:text-primary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 rounded-full h-8 w-8 md:h-9 md:w-9">
                  ...
                  {unreadCount > 0 && <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-white text-[10px] font-bold ring-2 ring-background">{unreadCount > 9 ? '9+' : unreadCount}</span>}
                  ...
                  <ScrollArea className="h-[300px] md:h-[400px]">
                    <div className="p-2 space-y-2">
                      {notifications.length === 0 ? (
                         <div className="p-4 text-sm text-muted-foreground text-center">Nenhuma notificação nova.</div>
                      ) : (
                         notifications.map(notification => {
                           ...
                           return (
                             <div
                               key={notification.id}
                               className={cn("mb-2 p-3 rounded-md border text-xs relative transition-colors duration-150 cursor-pointer", style.border, !notification.read ? style.bg : 'bg-secondary/50 border-border/30', 'hover:bg-accent/50')}
